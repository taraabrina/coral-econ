# ==============================================================================
# v3  - Refactored to use linear algebra for optimization
#     - Added 7 scenarios 
# ==============================================================================
...
# List reefs for group intervention (default:active candidates)
#preselected_ids <- c("2566","2539")
...
# Constants
LARVAE_PER_HCC <- 1000000
MATURITY_RATE  <- 0.09
COLONY_TO_KMSQ <- 0.0000000002
STEPS <- 25
RESTORATION_LARVAE <- 1.4e9

# ==============================================================================
# SIMULATION FUNCTION
# ==============================================================================
run_core_logic_ts <- function(hcc_start, steps, p_vec, r_indices=NULL, r_larvae=0) {
  ts_matrix <- matrix(0, nrow = length(hcc_start), ncol = steps)
  current_hcc <- hcc_start
  
  for (t in 1:steps) {
    larvae <- current_hcc * LARVAE_PER_HCC
    if (t == 1 && !is.null(r_indices)) {
      larvae[r_indices] <- larvae[r_indices] + r_larvae
    }
...
s1_ts <- run_core_logic_ts(HCC_INIT, STEPS, PENALTY)
...
individual_results <- foreach(idx = ph_indices, .options.snow = list(preschedule = TRUE)) %dopar% {
  ...
  s2_ts <- run_core_logic_ts(HCC_INIT, STEPS, PENALTY, r_indices = idx, r_larvae = RESTORATION_LARVAE)
  s3_ts <- run_core_logic_ts(HCC_INIT, STEPS, local_penalty)
  s4_ts <- run_core_logic_ts(HCC_INIT, STEPS, local_penalty, r_indices = idx, r_larvae = RESTORATION_LARVAE)

  list(
    final = c(s2_ts[idx, STEPS], s3_ts[idx, STEPS], s4_ts[idx, STEPS]),
    ts    = rbind(s2_ts[idx, ], s3_ts[idx, ], s4_ts[idx, ])
  )
}
stopCluster(cl)
...
s5_ts <- run_core_logic_ts(HCC_INIT, STEPS, PENALTY, r_indices = target_indices, r_larvae = RESTORATION_LARVAE)
s6_ts <- run_core_logic_ts(HCC_INIT, STEPS, portfolio_penalty)
s7_ts <- run_core_logic_ts(HCC_INIT, STEPS, portfolio_penalty, r_indices = target_indices, r_larvae = RESTORATION_LARVAE)
...
indiv_reefgain_ph_boxplot <- ggplot(indiv_reefgain_long, aes(x = intervention, y = gain, fill = intervention)) +
  ...
  labs(title = "Individual Reef Potential (S2-S4)", y = paste0("Gain in HCC (Yr ", STEPS, ")")) +
  theme_minimal()
...
# EXPORT
# ==============================================================================
output_file <- paste0("output/df_ph_yearly_hc-", format(Sys.Date(), "%Y%m%d"), ".xlsx")
write.xlsx(
  list(
    "Yearly_HCC"                = reef_yearly_ts,
    "Individual Intervention Gain" = indiv_reefgain_df,
    "Group Intervention Gain"   = group_reefgain_df
  ),
  file = output_file
)
